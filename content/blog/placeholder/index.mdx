...
> [!NOTE]
> When viewing a conversation, you can automatically quote text in a comment by highlighting the text, then typing <kbd>R</kbd>. You can quote an entire comment by clicking the kebab menu, then **Quote reply**. For more information about keyboard shortcuts, see [Keyboard shortcuts](/en/get-started/accessibility/keyboard-shortcuts).
...
| ----- | --------------------------- | ----------------------------------- | ------ |
| HEX   | <code>`#RRGGBB`</code>      | <code>`#0969DA`</code>              | HEX color preview |
| RGB   | <code>`rgb(R,G,B)`</code>   | <code>`rgb(9, 105, 218)`</code>     | RGB color preview |
| HSL   | <code>`hsl(H,S,L)`</code>   | <code>`hsl(212, 92%, 45%)`</code>   | HSL color preview |
...
You can link directly to any section that has a heading. To view the automatically generated anchor in a rendered file, hover over the section heading to expose the link icon and click the icon to display the anchor in your browser.
...
If you're writing in issues, pull requests, and discussions in a repository, GitHub will render a line break automatically:
...
![Image embedded in Markdown.](https://picsum.photos/seed/embed/800/600)
...
Example — here is a simple footnote[^1]. A footnote can also have multiple lines[^2].
